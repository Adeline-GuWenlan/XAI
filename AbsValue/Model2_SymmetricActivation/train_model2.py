"""
Model 2: MLP with Symmetric Activation (Tanh) + ReLU
Training script for MLP with symmetric activation function
"""
...
class MLPSymmetricActivation(nn.Module):
    ...
    def __init__(self, input_dim, hidden_dim=64, dropout_rate=0.0, output_dim=1):
        ...
        self.model = nn.Sequential(
            ...
            nn.Tanh(),                              # Symmetric activation (Tanh)
...
def main():
    ...
    model = MLPSymmetricActivation(input_dim, hidden_dim=HIDDEN_DIM, dropout_rate=DROPOUT_RATE).to(device)
    ...
    plt.xlabel('Epoch')
    plt.ylabel('MSE Loss')
    plt.title('Model 2: MLP with Tanh (Symmetric) + ReLU - Training Curves')
    plt.legend()
    plt.grid(True)
    ...
    # Save training log
    log = {
        "model": "MLP with Symmetric Activation (Tanh) + ReLU",
        "timestamp": timestamp,
        "device": str(device),
        "hyperparameters": {
    ...
    summary = f"""
Model 2: MLP with Symmetric Activation (Tanh) + ReLU - Training Summary
========================================================================
Timestamp: {timestamp}
Device: {device}
