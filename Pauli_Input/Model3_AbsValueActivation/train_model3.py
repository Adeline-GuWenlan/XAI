"""
Model 3: MLP with Absolute Value Activation + ReLU
Training script for MLP with custom absolute value activation function
"""
...
class AbsActivation(nn.Module):
    """Custom absolute value activation function: f(x) = |x|"""
    def __init__(self):
...
class MLPAbsActivation(nn.Module):
    """MLP with Absolute Value Activation + ReLU"""
    def __init__(self, input_dim, hidden_dim=64, dropout_rate=0.0, output_dim=1):
        super(MLPAbsActivation, self).__init__()

        self.model = nn.Sequential(
            nn.Linear(input_dim, hidden_dim),      # MLP layer 1
            AbsActivation(),                        # Absolute value activation
...
def main():
    ...
    model = MLPAbsActivation(input_dim, hidden_dim=HIDDEN_DIM, dropout_rate=DROPOUT_RATE).to(device)
    ...
    plt.xlabel('Epoch')
    plt.ylabel('MSE Loss')
    plt.title('Model 3: MLP with Abs + ReLU - Training Curves')
    plt.legend()
    plt.grid(True)
    ...
    # Save training log
    log = {
        "model": "MLP with Absolute Value Activation + ReLU",
        "timestamp": timestamp,
        "device": str(device),
        "hyperparameters": {
    ...
    summary = f"""
Model 3: MLP with Absolute Value Activation + ReLU - Training Summary
======================================================================
Timestamp: {timestamp}
Device: {device}
